...
function degreeFinder(){
  ...
  checkedDept = new Array();
  checkedNum = new Array();
  var i = 0;
  var j = 0;
  var k = 0;
  var z = 0
  son = new Array();
  son[0] = JSON.sessionId = window.localStorage.getItem('sessionId')
  var checkCount = 0;
  deptTaken = new Array();
  for(z; z< checkBoxes.length; z++){
    var box_checked = document.getElementById(checkBoxes[z]).checked;
    if(box_checked){
      for(i; i < checkedArray.length; i++){
        k = JSON.deptTaken.length;
        for(j; j < k; j++){
          if(son.deptTaken[j].name == checkedDept){
            k=j;
          }
        }
        son.deptTaken[k].name = checkedDept[i];
        son.deptTaken[k].coursesTaken[checkedArray].dept = checkedDept[i];
        son.deptTaken[k].coursesTaken[checkedArray].num = checkedNum[i];
      }
    }
  son.currDept.name = dept;
  son.currDept.type = type;
  son.semLeft = sems_left;
  son.genEdsLeft = gen_eds_left;
